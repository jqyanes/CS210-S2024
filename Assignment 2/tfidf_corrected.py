...
# Clean.
    ...
    #Original errors: 
    # did not remove Unicode characters that were not in the alphabet (e.g. ')
    # included "empty words" (e.g. "human - caused" would be converted to "human  caused")
                            if re.search(r'^http://.*', word) or re.search(r'^https://.*', word):
                                continue
                            res = re.sub(r'[^a-zA-Z0-9_]', '', word)
                            if word == '' or res == '':
                                continue
